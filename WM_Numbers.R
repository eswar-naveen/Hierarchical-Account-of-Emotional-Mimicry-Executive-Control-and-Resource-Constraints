...
data <- read_csv("rawdata_numbers6.csv", show_col_types = FALSE) %>%
...
write_csv(data_fixed, "Cleaned_Interpolated_Data_Fixed_Exp2.csv")
...
participant_summaries <- data_fixed %>%
  ...
  summarise(across(where(is.numeric), 
                   ...
                   .names = "{.col}_{.fn}"))
write_csv(participant_summaries, "participant_Mean_Median_Exp2.csv")
...
write_csv(data_downsized, "Downsized_Averages_with_Timestamps_Exp2.csv")

averaged_by_timestamp <- data_downsized %>%
  group_by(Timestamp) %>%
  summarise(across(where(is.numeric) & !any_of("participant"), ~mean(.x, na.rm = TRUE)))
write_csv(averaged_by_timestamp, "Averages_By_Timestamp_Exp2.csv")
...
binned_data %>%
  ...
  walk(~write_csv(.x %>% select(-Condition), paste0("Binned_", unique(.x$Condition), "_Fixed_Exp2.csv")))
...
write_csv(all_results, "Cluster_ANOVA_Results_ZM_Fixed_Exp2.csv")
...
write_csv(anova_results_df, "ANOVA_Results_ZM_Fixed_Exp2.csv")
...
ggsave("Figure8_Fstats_Cluster.png", plot = fstat_final, width = 10, height = 8, dpi = 300)
...
ggsave("Figure7_ZM_Lineplots.png", plot = figure_line, width = 12, height = 5, dpi = 300)
...
figure_cond <- ggplot(zm_means_time, aes(x = Bin, y = Mean_EMG_Value, color = Condition_Label, group = Condition_Label)) +
  ...
  theme_bw() +
  labs(title = "Condition Means Across Time Bins (ZM Muscle) - Exp2", 
       x = "Time Bin", y = "Mean EMG Value", color = "Condition") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom")

ggsave("FigureF6_ConditionMeans.png", plot = figure_cond, width = 10, height = 6, dpi = 300)

cat("--- Finished Analysis for Exp2 ---\n")